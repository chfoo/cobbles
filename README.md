...
You will need the dll files match the architecture of the HashLink exe. If you downloaded HashLink from the Haxe website, you will need 64 bit versions. The dlls can be installed to your system and bundled within the same directory as your application.

If prebuilt dlls are not available or you want to build them yourself, follow the build directions for Harfbuzz (which includes Freetype).

TODO: describe how to for iconv

#### Mac OS

Freetype is likely installed, but Harfbuzz may not.

If prebuilt libraries are not available or you want to build them yourself, follow the build directions for Harfbuzz (which includes Freetype). I suggest using the Homebrew package manager.
...
On Windows, instead of "make", use:
    ...
    mingw32-make.exe hdll WIN_PREFIX=/c/path/to/libraries/

Inspect the makefile to see the exact paths needed.
...
Remember that you must comply with the Freetype, Harfbuzz, Iconv licenses as well when bundling them.
